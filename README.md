...
```bash
conda create -n dbx_1 python=3.9
conda activate dbx_1
```
...
```
This command will create your personal repository under `/Repos/<username>/dbx_1`.
3. To set up the CI/CD pipeline with the notebook, create a separate `Staging` repo:
```bash
databricks repos create --url <your repo URL> --provider <your-provider> --path /Repos/Staging/dbx_1
```
...
- To trigger the release pipeline, get the current version from the `dbx_1/__init__.py` file and tag the current code version:
```
